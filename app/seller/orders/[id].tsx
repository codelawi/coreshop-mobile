  ...
  return (
    <SafeAreaView className="flex-1 bg-bg-light">
      <View className="flex-row items-center gap-3 px-6 py-4">
        ...
        <Text variant="bold" className="flex-1 text-xl text-brand">Order #{order.id}</Text>
        <View
      ...
      >
        ...
        <Animated.View entering={FadeInDown.duration(300)} className="rounded-xl bg-white p-4">
          ...
          {order.client ? (
            <View className="flex-row items-center gap-3">
              <View className="h-10 w-10 items-center justify-center rounded-full bg-brand-50">
                <HugeiconsIcon icon={User01Icon} size={20} color="#0A0A0A" />
              </View>
              <View className="flex-1">
                <Text variant="semibold" className="text-sm text-brand">{order.client.name}</Text>
                {order.client.phone ? (
                  <Text className="text-xs" style={{ color: "#6B7280" }}>{order.client.phone}</Text>
                ) : null}
              </View>
            </View>
          ) : null}
          ...
          {order.address ? (
            <View className="flex-row items-start gap-3">
              <View className="mt-0.5 h-10 w-10 items-center justify-center rounded-full bg-brand-50">
                <HugeiconsIcon icon={Location01Icon} size={20} color="#0A0A0A" />
              </View>
              <View className="flex-1">
                ...
                {order.address.phone ? (
                  <Text className="text-xs" style={{ color: "#6B7280" }}>{order.address.phone}</Text>
                ) : null}
              </View>
            </View>
          ) : null}
                ...
                <View className="h-14 w-14 overflow-hidden rounded-xl bg-brand-50">
                  {item.product_image ? (
                    ...
                    <View className="flex-1 items-center justify-center">
                      <HugeiconsIcon icon={Package01Icon} size={20} color="#9CA3AF" />
                    </View>
                  )}
                </View>
                <View className="flex-1">
                  <Text variant="medium" className="text-sm text-brand" numberOfLines={2}>
                    {item.product_name}
                  </Text>
                  {item.variant_label ? (
        ...
        >
          ...
          <View className="flex-row justify-between">
            ...
            <Text className="text-sm text-brand">JOD {Number(order.subtotal).toFixed(2)}</Text>
          </View>

          {Number(order.discount) > 0 ? (
            <View className="flex-row justify-between">
              <Text className="text-sm" style={{ color: "#6B7280" }}>Discount</Text>
          ...
          <View className="flex-row justify-between">
            <Text className="text-sm" style={{ color: "#6B7280" }}>Delivery fee</Text>
            <Text className="text-sm text-brand">JOD {Number(order.delivery_fee).toFixed(2)}</Text>
          </View>

          <View className="mt-1 h-px bg-brand-100" />

          <View className="flex-row justify-between">
            <Text variant="bold" className="text-sm text-brand">Total</Text>
          ...
          <View className="mt-1 flex-row items-center justify-between">
            <Text className="text-xs" style={{ color: "#6B7280" }}>Payment method</Text>
            <Text
              variant="medium"
        ...
        {order.notes ? (
          <Animated.View
            ...
            className="rounded-xl bg-white p-4"
          >
            <Text variant="semibold" className="mb-1 text-sm text-brand">Note from customer</Text>
            <Text className="text-sm leading-5" style={{ color: "#374151" }}>{order.notes}</Text>
          </Animated.View>
        ) : null}
      </ScrollView>
      ...
      {action ? (
        <View
          className="absolute bottom-0 left-0 right-0 border-t border-brand-100 bg-white px-6 pb-8 pt-4"
        >
          <Pressable
