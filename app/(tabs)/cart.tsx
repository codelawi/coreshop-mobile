    ...
    return (
      <SafeAreaView className="flex-1 bg-bg-light">
        <View className="px-6 pt-4">
          <Text variant="bold" className="text-2xl text-brand">Cart</Text>
        </View>
        <Animated.View
        ...
        >
          <View className="h-24 w-24 items-center justify-center rounded-full bg-brand-50">
            <HugeiconsIcon icon={ShoppingCart01Icon} size={40} color="#0A0A0A" />
          </View>
          <Text variant="bold" className="mt-4 text-lg text-brand">Your cart is empty</Text>
  ...
  return (
    <SafeAreaView className="flex-1 bg-bg-light">
      ...
      <Animated.View
        ...
        className="mx-6 mt-2 flex-row items-center gap-2 rounded-md bg-white p-3"
      >
        ...
        <View className="flex-1">
          <Text className="text-xs" style={{ color: "#6B7280" }}>Ordering from</Text>
          <Text variant="semibold" className="text-sm text-brand">{storeName}</Text>
        </View>
      </Animated.View>
      ...
      >
        {items.map((item, i) => (
          <Animated.View
            ...
            className="mb-3 flex-row gap-3 rounded-md bg-white p-3"
          >
            <View className="h-20 w-20 overflow-hidden rounded-md bg-brand-50">
              {item.image ? (
            ...
            <View className="flex-1">
              <View className="flex-row items-start justify-between gap-2">
                <Text variant="semibold" numberOfLines={2} className="flex-1 text-sm text-brand">
                  {item.name}
                </Text>
              ...
              <View className="mt-2 flex-row items-center justify-between">
                ...
                <View className="flex-row items-center gap-2">
                  <Pressable
                    ...
                    className="h-8 w-8 items-center justify-center rounded-md border border-brand-100"
                  >
                    <HugeiconsIcon icon={MinusSignIcon} size={14} color="#0A0A0A" />
                  </Pressable>
                  <Text variant="bold" className="min-w-[24px] text-center text-sm text-brand">
                    {item.quantity}
                  </Text>
                  <Pressable
                    onPress={() => setQty(item.id, item.quantity + 1)}
                  ...
                  >
                    <HugeiconsIcon icon={PlusSignIcon} size={14} color="#0A0A0A" />
                  </Pressable>
                </View>
              </View>
            </View>
      ...
      <SafeAreaView edges={["bottom"]} className="bg-white">
        <Animated.View
          ...
          className="border-t border-brand-100 px-6 py-4"
        >
          ...
          <Text className="mb-3 text-xs" style={{ color: "#6B7280" }}>
